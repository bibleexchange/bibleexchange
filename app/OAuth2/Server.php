...
namespace BibleExperience\OAuth2;

use BibleExperience\OAuth2\Controller\ResourceControllerInterface;
use BibleExperience\OAuth2\Controller\ResourceController;
use BibleExperience\OAuth2\OpenID\Controller\UserInfoControllerInterface;
use BibleExperience\OAuth2\OpenID\Controller\UserInfoController;
use BibleExperience\OAuth2\OpenID\Controller\AuthorizeController as OpenIDAuthorizeController;
use BibleExperience\OAuth2\OpenID\ResponseType\AuthorizationCode as OpenIDAuthorizationCodeResponseType;
use BibleExperience\OAuth2\OpenID\Storage\AuthorizationCodeInterface as OpenIDAuthorizationCodeInterface;
use BibleExperience\OAuth2\OpenID\GrantType\AuthorizationCode as OpenIDAuthorizationCodeGrantType;
use BibleExperience\OAuth2\Controller\AuthorizeControllerInterface;
use BibleExperience\OAuth2\Controller\AuthorizeController;
use BibleExperience\OAuth2\Controller\TokenControllerInterface;
use BibleExperience\OAuth2\Controller\TokenController;
use BibleExperience\OAuth2\ClientAssertionType\ClientAssertionTypeInterface;
use BibleExperience\OAuth2\ClientAssertionType\HttpBasic;
use BibleExperience\OAuth2\ResponseType\ResponseTypeInterface;
use BibleExperience\OAuth2\ResponseType\AuthorizationCode as AuthorizationCodeResponseType;
use BibleExperience\OAuth2\ResponseType\AccessToken;
use BibleExperience\OAuth2\ResponseType\JwtAccessToken;
use BibleExperience\OAuth2\OpenID\ResponseType\CodeIdToken;
use BibleExperience\OAuth2\OpenID\ResponseType\IdToken;
use BibleExperience\OAuth2\OpenID\ResponseType\IdTokenToken;
use BibleExperience\OAuth2\TokenType\TokenTypeInterface;
use BibleExperience\OAuth2\TokenType\Bearer;
use BibleExperience\OAuth2\GrantType\GrantTypeInterface;
use BibleExperience\OAuth2\GrantType\UserCredentials;
use BibleExperience\OAuth2\GrantType\ClientCredentials;
use BibleExperience\OAuth2\GrantType\RefreshToken;
use BibleExperience\OAuth2\GrantType\AuthorizationCode;
use BibleExperience\OAuth2\Storage\JwtAccessToken as JwtAccessTokenStorage;
use BibleExperience\OAuth2\Storage\JwtAccessTokenInterface;

/**
* Server class for OAuth2
* This class serves as a convience class which wraps the other Controller classes
*
* @see BibleExperience\OAuth2\Controller\ResourceController
* @see BibleExperience\OAuth2\Controller\AuthorizeController
* @see BibleExperience\OAuth2\Controller\TokenController
*/
    ...
    UserInfoControllerInterface
{
    ...
    protected $storageMap = array(
        'access_token' => 'BibleExperience\OAuth2\Storage\AccessTokenInterface',
        'authorization_code' => 'BibleExperience\OAuth2\Storage\AuthorizationCodeInterface',
        'client_credentials' => 'BibleExperience\OAuth2\Storage\ClientCredentialsInterface',
        'client' => 'BibleExperience\OAuth2\Storage\ClientInterface',
        'refresh_token' => 'BibleExperience\OAuth2\Storage\RefreshTokenInterface',
        'user_credentials' => 'BibleExperience\OAuth2\Storage\UserCredentialsInterface',
        'user_claims' => 'BibleExperience\OAuth2\OpenID\Storage\UserClaimsInterface',
        'public_key' => 'BibleExperience\OAuth2\Storage\PublicKeyInterface',
        'jwt_bearer' => 'BibleExperience\OAuth2\Storage\JWTBearerInterface',
        'scope' => 'BibleExperience\OAuth2\Storage\ScopeInterface',
    );

    protected $responseTypeMap = array(
        'token' => 'BibleExperience\OAuth2\ResponseType\AccessTokenInterface',
        'code' => 'BibleExperience\OAuth2\ResponseType\AuthorizationCodeInterface',
        'id_token' => 'BibleExperience\OAuth2\OpenID\ResponseType\IdTokenInterface',
        'id_token token' => 'BibleExperience\OAuth2\OpenID\ResponseType\IdTokenTokenInterface',
        'code id_token' => 'BibleExperience\OAuth2\OpenID\ResponseType\CodeIdTokenInterface',
    );

    /**
     * @param mixed                                                   $storage             (array or BibleExperience\OAuth2\Storage) - single object or array of objects implementing the
     ...
     * @param array                                                   $grantTypes          An array of BibleExperience\OAuth2\GrantType\GrantTypeInterface to use for granting access tokens
     * @param array                                                   $responseTypes       Response types to use.  array keys should be "code" and and "token" for
     *                                                                                     Access Token and Authorization Code response types
     * @param BibleExperience\OAuth2\TokenType\TokenTypeInterface                     $tokenType           The token type object to use. Valid token types are "bearer" and "mac"
     * @param BibleExperience\OAuth2\ScopeInterface                                   $scopeUtil           The scope utility class to use to validate scope
     * @param BibleExperience\OAuth2\ClientAssertionType\ClientAssertionTypeInterface $clientAssertionType The method in which to verify the client identity.  Default is HttpBasic
     *
     ...
     *
     * @param $request - BibleExperience\OAuth2\RequestInterface
     * Request object to grant access token
     *
     * @param $response - BibleExperience\OAuth2\ResponseInterface
    ...
    public function addStorage($storage, $key = null)
    {
        ...
        if (isset($this->storageMap[$key])) {
            ...
            if ($key === 'client' && !isset($this->storages['client_credentials'])) {
                if ($storage instanceof \BibleExperience\OAuth2\Storage\ClientCredentialsInterface) {
                    $this->storages['client_credentials'] = $storage;
                }
            } elseif ($key === 'client_credentials' && !isset($this->storages['client'])) {
                if ($storage instanceof \BibleExperience\OAuth2\Storage\ClientInterface) {
    ...
    protected function createDefaultAuthorizeController()
    {
        if (!isset($this->storages['client'])) {
            throw new \LogicException("You must supply a storage object implementing BibleExperience\OAuth2\Storage\ClientInterface to use the authorize server");
        }
    ...
    protected function createDefaultTokenController()
    {
            ...
            foreach ($this->grantTypes as $grantType) {
                if (!$grantType instanceof ClientAssertionTypeInterface) {
                    if (!isset($this->storages['client_credentials'])) {
                        throw new \LogicException("You must supply a storage object implementing BibleExperience\OAuth2\Storage\ClientCredentialsInterface to use the token server");
                    }
        ...
        if (!isset($this->storages['client'])) {
            throw new \LogicException("You must supply a storage object implementing BibleExperience\OAuth2\Storage\ClientInterface to use the token server");
        }
    ...
    protected function createDefaultResourceController()
    {
        if ($this->config['use_jwt_access_tokens']) {
            ...
            throw new \LogicException("You must supply a storage object implementing BibleExperience\OAuth2\Storage\AccessTokenInterface or use JwtAccessTokens to use the resource server");
        }
    ...
    protected function createDefaultUserInfoController()
    {
        if ($this->config['use_jwt_access_tokens']) {
            ...
            throw new \LogicException("You must supply a storage object implementing BibleExperience\OAuth2\Storage\AccessTokenInterface or use JwtAccessTokens to use the UserInfo server");
        }

        if (!isset($this->storages['user_claims'])) {
            throw new \LogicException("You must supply a storage object implementing BibleExperience\OAuth2\OpenID\Storage\UserClaimsInterface to use the UserInfo server");
        }
    ...
    protected function getDefaultResponseTypes()
    {
        ...
        if (isset($this->storages['authorization_code'])) {
            ...
            if ($this->config['use_openid_connect']) {
                if (!$this->storages['authorization_code'] instanceof OpenIDAuthorizationCodeInterface) {
                    throw new \LogicException("Your authorization_code storage must implement BibleExperience\OAuth2\OpenID\Storage\AuthorizationCodeInterface to work when 'use_openid_connect' is true");
                }
        ...
        if (count($responseTypes) == 0) {
            throw new \LogicException("You must supply an array of response_types in the constructor or implement a BibleExperience\OAuth2\Storage\AuthorizationCodeInterface storage object or set 'allow_implicit' to true and implement a BibleExperience\OAuth2\Storage\AccessTokenInterface storage object");
        }
    ...
    protected function createDefaultJwtAccessTokenStorage()
    {
        if (!isset($this->storages['public_key'])) {
            throw new \LogicException("You must supply a storage object implementing BibleExperience\OAuth2\Storage\PublicKeyInterface to use crypto tokens");
        }
        $tokenStorage = null;
    ...
    {
        if (!isset($this->storages['access_token'])) {
            throw new \LogicException("You must supply a response type implementing BibleExperience\OAuth2\ResponseType\AccessTokenInterface, or a storage object implementing BibleExperience\OAuth2\Storage\AccessTokenInterface to use the token server");
        }
    ...
    {
        if (!isset($this->storages['user_claims'])) {
            throw new \LogicException("You must supply a storage object implementing BibleExperience\OAuth2\OpenID\Storage\UserClaimsInterface to use openid connect");
        }
        if (!isset($this->storages['public_key'])) {
            throw new \LogicException("You must supply a storage object implementing BibleExperience\OAuth2\Storage\PublicKeyInterface to use openid connect");
        }
