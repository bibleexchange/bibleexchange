<?php namespace BibleExchange\GraphQL\Query;
    ...
    class NotesQuery extends Query {
        ...
        public function resolve($root, $args)
        {
            ...
            if(isset($args['id']))
            {
				//http://localhost/graphql?query=query+FetchNotes{notes(id:%227%22){id,body}}
                return Note::where('id' , $args['id'])->get();
            }
            else if(isset($args['bible_verse_id']))
            {
				//http://localhost/graphql?query=query+FetchNotes{notes(bible_verse_id:%2254004012%22){id,body}}
