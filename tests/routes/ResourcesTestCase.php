...
abstract class ResourcesTestCase extends StatementsTestCase {
  ...
  protected function constructData($data) {
    $data['lrs_id'] = new \MongoId($this->lrs->id);
    return $data;
  }
  ...
  public function testUpdate() {
    ...
    $response = $this->requestResource('PUT', static::$endpoint.'/'.$this->model->id, json_encode($data));
    $model = $this->getModelFromResponse($response);
  ...
  public function testShow() {
    $response = $this->requestResource('GET', static::$endpoint.'/'.$this->model->id);
    $model = $this->getModelFromResponse($response);
  ...
  public function testDestroy() {
    $response = $this->requestResource('DELETE', static::$endpoint.'/'.$this->model->id);
    $content = $this->getContentFromResponse($response);
