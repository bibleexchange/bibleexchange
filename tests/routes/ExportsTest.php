...
class ExportsTest extends ResourcesTestCase {
  ...
  protected function createReport() {
    $model = new \Report([
      ...
      'lrs_id' => new \MongoId($this->lrs->id)
    ]);
    $model->save();
    return $model;
  }
  ...
  public function testJson() {
    $response = $this->requestResource('GET', static::$endpoint.'/'."{$this->model->id}/show");
  ...
  public function testCsv() {
    $response = $this->requestResource('GET', static::$endpoint.'/'."{$this->model->id}/show/csv");
    $content = $response->getContent();
