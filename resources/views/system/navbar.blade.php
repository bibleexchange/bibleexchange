<ul class="nav navbar-nav navbar-right">
  ...
  <li class="dropdown">
    ...
    <ul class="dropdown-menu">
      @if( isset( $list ) )
        @foreach( $list as $lrs )
          <li><a href="{{ URL() }}/lrs/{{ $lrs->id }}"><i class='icon icon-bar-chart'></i> {{ $lrs->title }}</a></li>
        @endforeach
      @else
  ...
  <li><a href="{{ URL() }}/users/{{ Auth::user()->id }}/edit"><i class="icon icon-cog"></i> {{ trans('site.settings') }}</a></li>
