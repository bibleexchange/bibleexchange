...
@section('content')
  ...
  @foreach($lrs as $l)
    <?php
      $count = OldStatement::where('context.extensions.http://learninglocker&46;net/extensions/lrs.id', $l->id)->count();
      $count_new = Statement::where('lrs.id', $l->id)->count();
    ?>
    @if( $count > 0 )
    <li class="list-group-item">
      ...
      @if( !isset($count_new) || $count_new == 0 ) 
        <div class="migrate btn btn-primary btn-sm" data-lrs="{{$l->id}}"><i class="icon-play-circle"></i> Migrate</div>
      @endif
    </li>
    @endif
  @endforeach
